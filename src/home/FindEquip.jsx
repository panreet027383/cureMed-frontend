...
const FindEquip = () => {
 ...
 const [cityOptions, setCityOptions] = useState([]);
  ...
  const handleFind = async () => {
    if (!currcity || !equipmentName) {
      setError('⚠️ Please select a city and enter equipment name.');
      return;
    }

    try {
      const resp = await axios.post(server_url+"/needy/find-equip", {
        currcity, equipmentName
      });
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white rounded-xl shadow-lg">
      <h2 className="text-3xl font-bold mb-6 text-center text-blue-600">🛠️ Find Equipment</h2>
      ...
      <div className="flex flex-col md:flex-row items-center gap-4 mb-6">
        ...
        <button
          onClick={handleFind}
          className="bg-teal-600 hover:bg-teal-700 text-white px-6 py-3 rounded-full transition duration-300"
        >
          🔍 Search
        </button>
      </div>
          ...
          >
            <div>
              ...
              <div className="grid grid-cols-2 gap-2 text-sm text-gray-700">
                <p><span className="font-medium">📌 Purpose:</span> {equip.purpose}</p>
                <p><span className="font-medium">⚙️ Condition:</span> {equip.condition}</p>
                <p><span className="font-medium">📝 Issues:</span> {equip.issues}</p>
                <p className="col-span-2"><span className="font-medium">❗ Info:</span> {equip.info}</p>
              </div>
            </div>

            <button
              onClick={() => handleDetails(equip.emailid)}
